...
object TestHistory extends Properties("History Specification") {
	...
	property("Update is correct") = forAll(historyGen) { (input:(History[Int],Int)) => input match {
		case(history, inp) =>
			history.past == history.update(inp).past &&
			history.update(inp).present == inp
	}}
	
	property("Advance is correct") = forAll(historyGen) { (input:(History[Int],Int)) => input match {
		case(history, inp) =>
			(history.present :: history.past) == history.advance(inp).past &&
			history.update(inp).present == inp
	}}
	
	property("Undo is correct") = forAll(historyGen) { (input:(History[Int],Int)) => input match {
		case(history, _) =>
	...
	property("Redo is correct") = forAll(historyGen) { (input:(History[Int],Int)) => input match {
		case(history, _) =>
